...
- After the NAND was removed, the user reported continuity from PE9 to several U3 leads counted physically from the upper-right side, while the lead called "pin 11" no longer beeped. The schematic identifies U3/W9825G6KH DQ6 as actual package pin 11, but U3 pins 1, 3, and 9 are supply/VDDQ pins; physical edge counting is therefore not a valid pin-number reference. This observation remains unresolved until actual package pin 11 is measured in low-ohms mode and the removed NAND pads are checked for bridges.
- After the NAND was removed/reworked, the previously verified 50 MHz SDRAM diagnostic was flashed and captured on COM27. The board still failed identically for ReadPipe 0/1/2: `0xC0000008`, expected `851ABDC1`, actual `855ABDC1`, diff `00400000`. NAND removal therefore did not remove the D6 error; the current root cause remains in the SDRAM DQ6 path, MCU PE9 path, or PCB interconnect, subject to correct pin-level resistance/continuity verification.
