...
- Added the isolated `OPENMV_BOARD_DIAG` source path, official cascade data wrapper, board-side Haar/LBP implementation, RAM-only ID=1 enrollment, and a separate build script. Production mode remains unchanged pending compile and hardware verification.
- Production Release and separate board diagnostic builds both linked successfully. Board diagnostic size is `text=109508 data=96 bss=103296`; no YuNet/SFace deletion is required for this isolated test.
- Hardware proved board-side execution and FAV1 transport: no-face full pipeline was 82-95 ms; relaxed 10-stage detection produced face boxes at about 130 ms plus 3-4 ms LBP. One-shot raw-ROI LBP was unstable (`5816-9634` same-person distance), so candidate clustering and fixed 98x98 face normalization are being added before threshold calibration.
- Rejected the relaxed detector after user-observed false triggers. Strict 25-stage, equalized, 1.1-scale, five-neighbor detection produced repeated frontal-face results at 327-498 ms. Recognition was changed toward the validated PC contract: 15% margin, 128x128 equalized crop, 8x8 normalized 59-bin LBP, chi-square distance, and five-frame mean enrollment.
- Replaced intersection-only candidate grouping with OpenCV-style similar-rectangle grouping and changed the viewer so green means an actual ID match, not detection alone.
- Corrected board LBP to use raw grayscale rather than the globally equalized detector image. Hardware logs then separated same-person (`0.050..0.080`) from different-person (`0.130..0.241`) distances, so the board-only match threshold was recalibrated from `0.55` to `0.10`.
