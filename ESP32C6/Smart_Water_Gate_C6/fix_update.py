...
new = '''static void _do_update_all_params(void *priv)
{
    user_record_t *u = find_user_by_id(current_user_id);
    int uid = (current_user_id == FP_UNKNOWN) ? 0 : current_user_id;
    if (user_id_param) esp_rmaker_param_update_and_report(user_id_param, esp_rmaker_int(uid));
...
{
    esp_rmaker_work_queue_add_task(_do_update_all_params, NULL);
}'''
...
if old in content:
    ...
    print("NOT FOUND")
    idx = content.find('static void update_all_params')
    if idx > 0:
